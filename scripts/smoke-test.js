...
for (let room = debug.constants.ROOM_SELECT_VISIBLE_COUNT + 1; room <= 12; room += 1) {
...
if (debug.enemies.length !== 9 || debug.getActiveEnemies().length !== 0) {
  throw new Error("Level 1 Rooms 7, 8, and 9 should create one Walker each, Room 10 should create two Walkers, Room 11 should create two Walkers and two Jumpers, and Room 12 should add no enemies, all inactive while the player starts in Room 1.");
}
...
if (room9RightTransition?.targetRoomId !== "room-10"
  ...
  throw new Error("Room 10, Room 11, and Room 12 edge transitions were not configured correctly.");
}


debug.loadSelectedRoom("room-11");
const room11 = debug.getCurrentRoom();
const room11Ceiling = debug.platforms.find((platform) => platform.id === "room11-ceiling-platform");
const room11Floor = debug.platforms.find((platform) => platform.id === "room11-floor-platform");
if (!room11Ceiling || !room11Floor) throw new Error("Room 11 ceiling and floor platforms were not found.");
if (room11Ceiling.x !== room11.x || room11Ceiling.y !== 0 || room11Ceiling.w !== room11.w || room11Ceiling.h !== 24) {
  throw new Error("Room 11 ceiling should be completely covered by one full-width platform.");
}
if (room11Floor.x !== room11.x || room11Floor.y !== 470 || room11Floor.w !== room11.w || room11Floor.h !== 70) {
  throw new Error("Room 11 floor should be completely covered by one full-width platform.");
}
const room11Enemies = debug.enemies.filter((enemy) => enemy.roomId === "room-11");
const room11EnemyLabel = (enemy) => enemy.constructor.name === "Enemy" ? "Walker" : enemy.constructor.name;
if (room11Enemies.length !== 4) throw new Error(`Room 11 should have exactly four enemies, got ${room11Enemies.length}.`);
if (room11Enemies.filter((enemy) => room11EnemyLabel(enemy) === "Walker" && enemy.gravitySign === 1).length !== 1
  || room11Enemies.filter((enemy) => room11EnemyLabel(enemy) === "Jumper" && enemy.gravitySign === 1).length !== 1
  || room11Enemies.filter((enemy) => room11EnemyLabel(enemy) === "Walker" && enemy.gravitySign === -1).length !== 1
  || room11Enemies.filter((enemy) => room11EnemyLabel(enemy) === "Jumper" && enemy.gravitySign === -1).length !== 1) {
  throw new Error(`Room 11 should have a Walker and Jumper on the floor plus an upside-down Walker and Jumper on the ceiling; got ${room11Enemies.map((enemy) => `${room11EnemyLabel(enemy)}:${enemy.gravitySign}`).join(", ")}.`);
}
debug.loadSelectedRoom("room-1");
...
const expectedTriggerMessages = [
  ...
  "Proceed.",
  "Combat pattern stabilized.",
  "Checkpoint established.",
  "Unusual gravitational activity detected.",
...
debug.update(16 / 1000);
debug.enterRoom("room-12", { x: room12X + 18, y: 420 }, { grounded: true, facing: 1 });
for (const x of [room12X + 55, room12X + 320, room12X + 680, room12X + 910]) {
  debug.player.placeAt(x, 420, { grounded: true });
  debug.update(16 / 1000);
}
...
// Room 8 right edge connects to Room 9, and Room 9 can return left to Room 8.
...
if (debug.getCurrentRoomId() !== "room-9") {
  throw new Error(`Room 8 right-edge transition should enter Room 9; got ${debug.getCurrentRoomId()}.`);
}
if (debug.player.x < room9X || debug.player.x > room9X + 60) {
  throw new Error(`Room 9 entry should place the player at the left start, got x=${debug.player.x}.`);
}
if (debug.getActiveEnemies().length !== 1) {
  throw new Error("Room 9 should activate exactly one Walker.");
}
const activeRoom9Walker = debug.getActiveEnemies()[0];
if (activeRoom9Walker.roomId !== "room-9" || activeRoom9Walker.gravitySign !== 1 || activeRoom9Walker.defaultGravitySign !== 1) {
  throw new Error("Room 9 Walker should be the only active Walker and should spawn with normal default gravity.");
}

debug.player.placeAt(room9X, 420, { grounded: true });
dispatch("keydown", keyEvent("a", "KeyA"));
debug.checkRoomEdgeTransitions();
for (let frame = 0; frame < 24; frame += 1) debug.update(16 / 1000);
dispatch("keyup", keyEvent("a", "KeyA"));
...
// Room 9 right edge connects to Room 10, Room 10 connects onward to
// Room 11 connects to Room 12, and Room 12's right edge remains reserved for the future boss room.
...
if (debug.getCurrentRoomId() !== "room-11") {
  throw new Error(`Room 10 right-edge transition should enter Room 11; got ${debug.getCurrentRoomId()}.`);
}
if (debug.player.x < room11X || debug.player.x > room11X + 60) {
  throw new Error(`Room 11 entry should place the player at the left start, got x=${debug.player.x}.`);
}
if (debug.getActiveEnemies().length !== 4 || debug.getActiveEnemies().some((enemy) => enemy.roomId !== "room-11")) {
  throw new Error("Room 11 should activate exactly two Walkers and two Jumpers.");
}
debug.player.placeAt(room11X, 420, { grounded: true });
dispatch("keydown", keyEvent("a", "KeyA"));
debug.checkRoomEdgeTransitions();
for (let frame = 0; frame < 24; frame += 1) debug.update(16 / 1000);
dispatch("keyup", keyEvent("a", "KeyA"));
if (debug.getCurrentRoomId() !== "room-10") {
  throw new Error(`Room 11 left-edge transition should return to Room 10; got ${debug.getCurrentRoomId()}.`);
}
debug.enterRoom("room-11", { x: room11X + 900, y: 420 }, { grounded: true, facing: 1 });
...
if (debug.getActiveEnemies().length !== 0) {
  throw new Error("Room 12 should be quiet with no active enemies.");
}
const room12Floor = debug.platforms.find((platform) => platform.id === "room12-floor-platform");
if (!room12Floor || room12Floor.x !== room12X || room12Floor.y !== 470 || room12Floor.w !== 960 || room12Floor.h !== 70) {
  throw new Error("Room 12 should use one flat full-width floor platform.");
}
debug.player.placeAt(room12X, 420, { grounded: true });
dispatch("keydown", keyEvent("a", "KeyA"));
debug.checkRoomEdgeTransitions();
for (let frame = 0; frame < 24; frame += 1) debug.update(16 / 1000);
dispatch("keyup", keyEvent("a", "KeyA"));
if (debug.getCurrentRoomId() !== "room-11") {
  throw new Error(`Room 12 left-edge transition should return to Room 11; got ${debug.getCurrentRoomId()}.`);
}
debug.enterRoom("room-12", { x: room12X + 18, y: 420 }, { grounded: true, facing: 1 });
debug.player.placeAt(room12X + 320, 420, { grounded: true });
debug.update(16 / 1000);
if (debug.activeCheckpoint.roomId !== "room-12" || debug.activeCheckpoint.id !== "l1r12-checkpoint") {
  throw new Error("Room 12 checkpoint trigger did not become the active respawn point.");
}
const gravitySnapshot = debug.activeCheckpoint.abilitySnapshot?.abilities.find((ability) => ability.id === "gravity");
const lockedSnapshot = debug.activeCheckpoint.abilitySnapshot?.abilities.filter((ability) => ability.id !== "gravity" && ability.unlocked);
if (!gravitySnapshot?.unlocked || lockedSnapshot.length !== 0) {
  throw new Error("Room 12 checkpoint did not save the expected ability state.");
}
debug.player.hp = 0;
debug.player.fullRespawn();
if (debug.getCurrentRoomId() !== "room-12" || Math.abs(debug.player.x - debug.activeCheckpoint.spawn.x) > 0.001) {
  throw new Error("Full respawn should return to the active Room 12 checkpoint.");
}
...
room9Walker.onSurface = true;
room9Walker.groundedPlatform = room9Plate;
debug.update(16 / 1000);
